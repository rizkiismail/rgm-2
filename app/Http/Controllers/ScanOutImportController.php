...
use App\Services\ScanOutImporter;
use Illuminate\Http\Request;
use Illuminate\Support\Str;

class ScanOutImportController extends Controller
{
    public function form()
    {
        $lastImports = ScanOut::select('import_batch')
        ...
        return view('scanout.import', [
            'lastImports' => $lastImports,
            'totalData' => ScanOut::count(),
        ]);
    }

    public function store(Request $request, ScanOutImporter $importer)
    {
        $request->validate([
        ...
        return redirect()->route('scanout.dashboard')->with('success', $message);
    }
}
