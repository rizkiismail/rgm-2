...
class BalanceReturExcelExporter
{
    ...
    protected function writeMetricRow($sheet, int $row, string $label, int|float $value): int
    {
        $sheet->setCellValue("A{$row}", $label);
        $sheet->setCellValue("B{$row}", $value);
    ...
    protected function buildDetailSheet(Spreadsheet $spreadsheet, Collection $rows): void
    {
        ...
        $headers = [
            ...
            'Qty Retur', 'Unit', 'Qty Receiving', 'Status Receiving',
            'Qty Delivery', 'Status Delivery', 'Final Status', 'Note', 'PIC PPIC Delivery',
        ];

        $sheet->fromArray($headers, null, 'A1');
        $sheet->getStyle('A1:N1')->getFont()->setBold(true)->getColor()->setRGB('FFFFFF');
        $sheet->getStyle('A1:N1')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB(self::HEADER_FILL);
        ...
        foreach ($rows as $item) {
            $sheet->fromArray([
                ...
                $item->code_item,
                $item->part_name,
                (float) $item->qty_retur,
                $item->unit,
                (float) $item->qty_receiving_part,
                $item->status_receiving,
                (float) $item->qty_delivery_part,
